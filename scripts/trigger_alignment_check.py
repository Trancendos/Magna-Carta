...
# trigger_id -> substring that must appear in at least one activated framework's
# register name (case-insensitive). Only triggers named after a specific
# framework or framework family belong here; scope-named triggers
# (TRG-CORE-001, TRG-US-GOV-001, TRG-EU-INDUSTRY-001, TRG-INTL-ASSURANCE-001,
# TRG-GLOBAL-PRIVACY-001, TRG-PAYMENTS-001, TRG-AI-US-001) are excluded because
# no single framework name corresponds to them.
ANCHORS = {
    "TRG-GDPR-001": "GDPR",
    "TRG-PECR-001": "PECR",
    "TRG-ISO-001": "ISO",
...
def main() -> int:
    ...
    for trig in triggers:
        ...
        if anchor:
            ...
            if not any(anchor.lower() in n.lower() for n in names):
