...
def _extract_frameworks(frameworks_doc: dict, errors: list[str]) -> dict[str, str]:
    """Every record is validated, not filtered.

    A comprehension guarded by `isinstance(x, dict)` silently drops whatever it
    does not understand, so a register half-converted to some other shape would
    pass this check with most of it never examined — the opposite of what a
    register validator is for. The walk carries its index so a report names the
    offending item. `_extract_triggers` below does the same, for the same reason.
    """
